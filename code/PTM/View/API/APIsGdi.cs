namespace System.Runtime.InteropServices.APIs
{
	internal class APIsGdi
	{
		...
		[DllImport("gdi32.dll")]
		internal static extern bool StretchBlt(IntPtr hDCDest, int XOriginDest, int YOriginDest, int WidthDest, int HeightDest,
		                                       IntPtr hDCSrc, int XOriginScr, int YOriginSrc, int WidthScr, int HeightScr,
		                                       APIsEnums.PatBltTypes Rop);

		[DllImport("gdi32.dll")]
		internal static extern IntPtr CreateCompatibleDC(IntPtr hDC);

		[DllImport("gdi32.dll")]
		internal static extern IntPtr CreateCompatibleBitmap(IntPtr hDC, int Width, int Heigth);

		[DllImport("gdi32.dll")]
		internal static extern IntPtr SelectObject(IntPtr hDC, IntPtr hObject);

		[DllImport("gdi32.dll")]
		internal static extern bool BitBlt(IntPtr hDCDest, int XOriginDest, int YOriginDest, int WidthDest, int HeightDest,
		...
		[DllImport("gdi32.dll")]
		internal static extern bool PatBlt(IntPtr hDC, int XLeft, int YLeft, int Width, int Height, uint Rop);
		...
		[DllImport("gdi32.dll")]
		internal static extern int GetObjectType(IntPtr handle);
		...
		[DllImport("gdi32")]
		internal static extern int GetDIBits(IntPtr hDC, IntPtr hbm, int StartScan, int ScanLines, int lpBits,
		                                     APIsStructs.BITMAPINFOHEADER bmi, int usage);

		[DllImport("gdi32")]
		internal static extern int GetDIBits(IntPtr hdc, IntPtr hbm, int StartScan, int ScanLines, int lpBits,
		                                     ref APIsStructs.BITMAPINFO_FLAT bmi, int usage);
		...
		[DllImport("gdi32")]
		internal static extern int GetClipBox(IntPtr hDC,
		                                      ref WIN32Rect r);
	}
}