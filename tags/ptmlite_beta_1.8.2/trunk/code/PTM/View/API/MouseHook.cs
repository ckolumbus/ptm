	...
	public class MouseHook : LocalWindowsHook, IDisposable
	{
		...
		protected int MouseProc(int code, IntPtr wParam, IntPtr lParam)
		{
			...
			if (code == Win32.HC_ACTION)
				RaiseMouseHookEvent(wParam,
				                    CrackHookMsg(wParam,
				                                 (Win32.MOUSEHOOKSTRUCT)
				                                 Marshal.PtrToStructure(lParam, typeof (Win32.MOUSEHOOKSTRUCT))));
		...
		private MouseHookEventArgs CrackHookMsg(IntPtr wParam, Win32.MOUSEHOOKSTRUCT hookStruct)
		{
			...
			return
				new MouseHookEventArgs(button, hookStruct.pt.x, hookStruct.pt.y, control, (HitTestCode) hookStruct.wHitTestCode);
		}
		...
		private struct Win32
		{
			...
			public static ushort LOWORD(int l)
			{
				return (ushort) (l);
			}

			public static ushort HIWORD(int l)
			{
				return (ushort) (((int) (l) >> 16) & 0xFFFF);
			}
