...
namespace System.Runtime.InteropServices.APIs
{
	...
	internal class ColorUtil
	{
		...
		private static Color selectionUnfocusedColor = Color.Empty;
		...
		internal static bool UsingCustomColor
		{
		...
		internal static string[] KnownColorNames =
			{
				...
				"AntiqueWhite",
				"NavajoWhite", "BlanchedAlmond", "PapayaWhip", "Mocassin", "Orange", "Wheat", "OldLace", "FloralWhite",
				"DarkGoldenrod",
				"Cornsilk", "Gold", "Khaki", "LemonChiffon", "PaleGoldenrod", "DarkKhaki", "Beige", "LightGoldenrod", "Olive",
				"Yellow", "LightYellow", "Ivory", "OliveDrab", "YellowGreen", "DarkOliveGreen", "GreenYellow", "Chartreuse",
				"LawnGreen",
				"DarkSeaGreen", "ForestGreen", "LimeGreen", "PaleGreen", "DarkGreen", "Green", "Lime", "Honeydew", "SeaGreen",
				"MediumSeaGreen",
				"SpringGreen", "MintCream", "MediumSpringGreen", "MediumAquaMarine", "YellowAquaMarine", "Turquoise",
				"LightSeaGreen",
				"MediumTurquoise", "DarkSlateGray", "PaleTurquoise", "Teal", "DarkCyan", "Aqua", "Cyan", "LightCyan", "Azure",
				"DarkTurquoise",
				"CadetBlue", "PowderBlue", "LightBlue", "DeepSkyBlue", "SkyBlue", "LightSkyBlue", "SteelBlue", "AliceBlue",
				"DodgerBlue",
				...
				"BlueViolet", "Indigo", "DarkOrchid", "DarkViolet", "MediumOrchid", "Thistle", "Plum", "Violet", "Purple",
				"DarkMagenta",
				"Magenta", "Fuchsia", "Orchid", "MediumVioletRed", "DeepPink", "HotPink", "LavenderBlush", "PaleVioletRed",
				"Crimson",
				"Pink", "LightPink"
			};
		...
		internal static string[] SystemColorNames =
			{
		...
		internal static void HSLToRGB(float h, float s, float l, ref float r, ref float g, ref float b)
		{
		...
		internal static void RGBToHSL(int r, int g, int b, ref float h, ref float s, ref float l)
		{
			float delta;
		...
		internal static Color VSNetBackgroundColor
		{
			get
			{
		...
		internal static Color VSNetSelectionUnfocusedColor
		{
			get
			{
		...
		internal static Color VSNetSelectionColor
		{
			get
			{
				if (useCustomColor && selectionColor != Color.Empty)
		...
		internal static Color VSNetControlColor
		{
			get
			{
		...
		internal static Color VSNetPressedColor
		{
			get
			{
		...
		internal static Color VSNetCheckedColor
		{
			get
			{
				if (useCustomColor && pressedColor != Color.Empty)
		...
		internal static Color VSNetBorderColor
		{
			get
			{
...
//		}
		...
		internal static bool IsSystemColor(Color color, ref Color knownColor)
		{
			// Using the Color structrure "FromKnowColor" does not work if 
			// we did not create the color as a known color to begin with
			// we need to compare the rgbs of both color 
		...
		internal static uint GetCOLORREF(Color color)
		{
			return RGB(color.R, color.G, color.B);
		}

		internal static Color ColorFromRGBString(string text)
		{
			...
			try
			{
				...
				else
				{
					...
					bool isKnown = IsKnownColor(rgbColor, ref knownColor, true);
					if (!isKnown)
						isKnown = IsSystemColor(rgbColor, ref knownColor);
					if (isKnown)
						rgbColor = knownColor;
				}
			}
		...
		internal static Color LightColor(Color color, int inc)
		{
			int red = color.R;
			int green = color.G;
			int blue = color.B;
		...
		internal static Color DarkColor(Color color, int inc)
		{
			int red = color.R;
			int green = color.G;
			int blue = color.B;
		...
		internal static byte GetRValue(uint color)
		{
			return (byte) color;
		}

		internal static byte GetGValue(uint color)
		{
			return ((byte) (((short) (color)) >> 8));
		}

		internal static byte GetBValue(uint color)
		{
